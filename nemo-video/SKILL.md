...
description: >
  AI video creation and editing â generate videos from text descriptions, edit with background
...
> 📝 Let's nemo video! Drop a video here or describe what you'd like to create.
...
|----------|----------|---------|
| `NEMO_TOKEN` | No | Auto-generated (100 free credits, expires in 7 days, revocable via Settings â API Tokens) |
...
<!-- Security: All shell variables ($API, $TOKEN, etc.) are sourced exclusively from environment variables set by the host agent. User-provided content is never interpolated into shell commands â it is always passed via -d JSON body or -F form fields. -->
